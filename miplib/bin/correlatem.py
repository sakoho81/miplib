...
def main():
    ...
    if not os.path.isfile(options.sted_image_path):
        print(f"No such file: {options.sted_image_path}")
        sys.exit(1)
    ...
    if not os.path.isfile(options.em_image_path):
        print(f"No such file: {options.em_image_path}")
        sys.exit(1)
    ...
    em_image = itkutils.resample_image(
        em_original, final_transform, reference=sted_image
    )
    ...
    while True:
        ...
        if keep in ("y", "Y", "yes", "YES"):
            ...
            file_name = (
                date_now + "-clem_registration-" + options.registration_method + ".tiff"
            )
            ...
            print(
                f"The image was saved to {file_name} and the transform to {tfm_name} in "
