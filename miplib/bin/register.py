...
def main():
    ...
    if not os.path.exists(full_path):
        raise ValueError(f"The specified file {full_path} does not exist")
    ...
    for view in range(1, data.get_number_of_images(ImageType.ORIGINAL)):
        ...
        if options.evaluate_results:
            ...
            sitk.Show(itkutils.make_composite_rgb_image(fixed_image, moving_image))
            if utils.get_user_input("Do you want to save the result (yes/no)? "):
                task.save_result()
                continue
            else:
                if utils.get_user_input(
                    "Skipping view %i. Do you want to continue registration?" % view
                ):
                    continue
                else:
