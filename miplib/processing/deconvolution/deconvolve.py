...
class DeconvolutionRL:
    ...
    def __init__(self, image, psf, writer, options):
        ...
        if options.verbose:
            logger.info("The original image size is %s", self.image_size)
        ...
        if options.verbose:
            logger.info("The deconvolution will be run with %i blocks", self.num_blocks)
            logger.info("The internal block size is %s", padded_block_size)
    ...
    def compute_estimate(self):
        ...
        if self.options.verbose:
            logger.info(
                "Beginning the computation of the %i. estimate", self.iteration_count
            )
        ...
        for idx in itertools.product(*iterables):
            estimate_idx = tuple(
                slice(j, j + k) for j, k in zip(idx, self.block_size, strict=False)
            )
        ...
        if self.options.tv_lambda > 0 and self.iteration_count > 0:
            if self.estimate.ndim == 2:
                ...
                dv_est = ops_ext.div_unit_grad(
                    numpy.expand_dims(self.estimate, 0), spacing
                )[0]
            else:
    ...
    def execute(self):
        ...
        try:
            while True:
                if (
                    self.options.update_blind_psf > 0
                    and self.iteration_count > 0
                    and (self.iteration_count + 1) % self.options.update_blind_psf == 0
                ):
                ...
                if int(u) == 0 and int(n) == 0:
                    ...
                    if frc_diff <= self.options.rl_frc_stop:
                        logger.info(
                            "Desired FRC diff reached after %d iterations",
    ...
    def __get_psfs(self):
        ...
        zoom_factors = tuple(
            x / y for x, y in zip(self.psf_spacing, self.image_spacing, strict=False)
        )
    ...
    def __calculate_block_and_image_size(self):
        ...
        if self.imdims == 2:
            block_size = numpy.ceil(
                self.image_size.astype(numpy.float16) / multiplier2
            ).astype(numpy.int64)
            image_size += multiplier2 * block_size - image_size
        else:
            block_size = numpy.ceil(
                self.image_size.astype(numpy.float16) / multiplier3
            ).astype(numpy.int64)
