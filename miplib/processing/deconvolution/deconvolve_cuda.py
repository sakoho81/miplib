...
class DeconvolutionRLCuda(deconvolve.DeconvolutionRL):
    ...
    def __init__(self, image, psf, writer, options):
        ...
        self._fft_plan = fftpack.get_fft_plan(
            cp.zeros(self.block_size, dtype=cp.complex64)
        )
    ...
    def compute_estimate(self):
        ...
        for pos in itertools.product(*iterables):
            ...
            h_image_block = self.get_padded_block(self.image, index.copy()).astype(
                np.float32
            )
            ...
            h_estimate_block_new = self._fft_convolve(
                h_estimate_block_new, self.adj_psf_fft
            ).real
    ...
    def __get_fourier_psfs(self):
        ...
        adj_psf_fft = ops_array.expand_to_shape(adj_psf, padded_block_size).astype(
            np.complex64
        )
