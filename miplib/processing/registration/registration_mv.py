...
class RotatedMultiViewRegistration:
    ...
    def execute(self):
        ...
        modified_moving_image = ops_itk.resample_image(moving_image, manual_transform)
        ...
        print(
            "Starting registration of views "
        ...
        self.final_transform = sitk.AffineTransform(
            combined_mat.flatten(), combined_translation, combined_center
        )

        # Print final metric value and stopping condition
        print(f"Final metric value: {self.registration.GetMetricValue()}")
        print(
            f"Optimizer's stopping condition, {self.registration.GetOptimizerStopConditionDescription()}"
        )
    ...
    def save_result(self):
        ...
        self.data.add_transform(
            scale, view, channel, tfm_params[1], tfm_params[2], tfm_params[0]
        )
    ...
    def add_observers(self, start, update):
        ...
        self.registration.AddCommand(
            sitk.sitkIterationEvent, lambda: update(self.registration)
        )
    ...
    def execute(self):
        ...
        for idx in range(self.data.get_number_of_images(ImageType.ORIGINAL)):
            ...
            if moving_image.GetDimension() == 2:
                self.final_transform = registration.itk_registration_rigid_2d(
                    fixed_image, moving_image, self.options
                )
            else:
                self.final_transform = registration.itk_registration_rigid_3d(
                    fixed_image, moving_image, self.options
                )
            self.save_result()
