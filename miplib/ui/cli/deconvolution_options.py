...
def get_deconvolution_options_group(parser):
    ...
    group = parser.add_argument_group(
        "Deconvolution", "Options for controlling the deconvolution algorithm"
    )
    ...
    group.add_argument("--wiener-nsr", type=float, default=100.0)

    group.add_argument(
        "--first-estimate",
        choices=["image", "blurred", "image_mean", "constant"],
        default="image",
        help="Specify first estimate for iteration.",
    )

    group.add_argument(
        "--estimate-constant", dest="estimate_constant", type=float, default=1.0
    )
    ...
    group.add_argument("--memmap-estimates", action="store_true")
    ...
    group.add_argument("--rl-auto-background", action="store_true")
    group.add_argument(
        "--rl-frc-stop",
        type=float,
        default=0.0,
